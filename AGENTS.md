...
recorded matches), so the read is read-only and the run is destructive.
`get_rename_benchmark` scores a binary's stored rename proposals against the
names an ingested debug symbol file supplied (precision, recall, F1, every
disagreement and every missed symbol) and is a stored read, so it is read-only
and never answers a tool error for an input that is merely missing: it returns
``stored: false`` with the reason.  `list_scans` reads the stored scans of a
binary's newest analysis with the inputs each ran with (the result payload is
left out) and is read-only.  `get_symbols` reads a binary's ingested debug symbol
...
and `run_match` expose the same over MCP, and the counts stay 252 built-in
tools (120 read-only, 132 destructive).
...
- SPA is Vite + React + TypeScript in `web/`, built with bun into
  ...
  route; the shell's `Space` binding imports only `panels/codeViewSwitch.ts`,
  so `FunctionPanels` loads with the function-detail route), React and the
  router are one `vendor` chunk, and the smoke fails when a view's marker ends
  up in the entry bundle.  Hashed assets under `/static/assets/` are answered
  `immutable` and gzip-encoded when the client accepts gzip (a build-time
  `.gz` sibling from `scripts/precompress_spa.py`, else on-the-fly).  A table
  whose list is the stored rows themselves (Functions, Matches) carries
  `DataTable`'s `windowed` prop, which renders the rows around the viewport
  over a spacer row sized from the first row's measured height; it is opt-in
  because a table whose rows differ in height would drift.  `docs/SPA.md`
  ("Long tables") carries the measurements.
