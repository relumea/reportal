...
class TestRegistry:
    def test_builtin_tools_cover_every_capability(self) -> None:
        ...
        assert len(_READ_ONLY_TOOLS) == 127
        assert len(_DESTRUCTIVE_TOOLS) == 144
...
class TestReadTools:
    ...
    def test_list_binaries_filters_and_orders(self, conn: Any, tmp_path: Path) -> None:
        ...
        payload, is_error = _call("list_binaries", {"tag": "packed"})
        assert is_error is False
        assert [row["name"] for row in payload["binaries"]] == ["demo.exe"]
        assert payload["tag"] == "packed"
        assert payload["total"] == 2

        payload, is_error = _call("list_binaries", {"search": "cd", "order": "name"})
        assert is_error is False
