...
Two host tools make an upload useful, and `reportal doctor` warns without either:
`rizin` (function discovery, which the `analyse` job an upload queues runs) and
`kuna` (the default decompiler), which reads SLEIGH processor specs: the engine
finds the ones a pypcode install bundles on its own, and `KUNA_SPECS` in the
unit's environment overrides that with a Ghidra install root.

The AI extras need an OpenAI-compatible chat-completions endpoint and nothing
hosted: a local Ollama serves one, so `REPORTAL_LLM_ENDPOINT=http://127.0.0.1:11434/v1`
with `REPORTAL_LLM_MODEL` set to a pulled model (`gemma4:e2b-it-q8_0` was checked
against the summary and rename routes) keeps the model on the host.
