...
| Realms and interception | `Context.derive` gives a child context that reads the parent's bindings, writes into its own table and journals only its own effects, so one name resolves differently in two contexts; `Context.drop` reverts that realm and detaches it. `Context.intercept` wraps reads of one name and is itself a journaled effect a revert uninstalls |
| Fibers and inertia | `Fiber` is one instantiation: its parent fiber, its own coeffect table (a derived context by default), a retirement flag and the four lifecycle states, with each `provides` name committed to the fiber's view. The live `ComponentHost` hangs one fiber per activation off its root fiber; `Fiber.retire` deactivates children newest-first and returns an `Inertia` handle the withdrawal reports |
...
| Hot module replacement | `registrations()` records each entry's declaring module and reloadability; `reload_component(name)` re-imports it and swaps the entry in place, `reload_all()` reports one result per entry, and `pipeline.ComponentHost` drives deactivate → reload → activate against a live `Context`, retiring the fiber of any component whose declaration left the registry. A run in flight keeps its snapshot; `refresh_components()` remains the whole-registry re-discovery |
