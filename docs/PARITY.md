...
reportal's own surface for the comparison is its FastAPI schema (223
paths, 310 operations) plus the MCP tool registry (240 tools).  Every row below is
...
`get_data_type_functions` MCP tools (240 tools: 113 read-only, 127 destructive);
...
| 4 | **MCP destructive annotations on every writer** | 27 of the hosted server's 36 tools mutate state but only 9 carry a destructive marker (`docs.reveng.ai/mcp`) | Done: all 240 local tools carry `readOnlyHint` or `destructiveHint`, and a test pins the split |
...
| 11 | **VeriDecomp as a product surface** | "we are developing an internal benchmark called VeriDecomp" (`reveng.ai/blog/introducing-wilbert-and-ventris`) | Done for matching: `benchmark.py` scores a match run against labelled counterpart addresses (a corpus file, or the two binaries' shared real names) and reports precision, recall, F1, mean reciprocal rank and every miss, stored as the left binary's `benchmark` scan and reachable from `reportal benchmark`, `POST /api/binaries/<id>/benchmark`, the `run_benchmark` MCP tool and the Benchmark panel.  Rename proposals are deliberately not scored: their correctness needs a labelled name and a proposal source, so a name metric invented from the signature match would measure the label rather than the rename. |
...
in `docs/TODO.md` (no crawl entry asked for one).  Row 11 shipped the matching
half of its harness and states the rename half it does not score.  Row 1
