...
reportal's own surface for the comparison is its FastAPI schema (205
paths, 286 operations) plus the MCP tool registry (223 tools).  Every row below is
...
`get_data_type_functions` MCP tools (223 tools: 104 read-only, 119 destructive);
...
`list_conversation_runs` and `get_conversation_run` (223 tools: 104 read-only,
119 destructive); and the conversation detail's Agent run panel (the question
