...
| Binary download | Implemented | local store | `GET /api/binaries/<id>/download` streams a stored binary back as an attachment, reading it in 1 MiB chunks rather than whole (an upload may be up to the API's 256 MiB cap), so a large binary is never held in memory whole. The filename comes from the sanitized stored name (one path component, everything outside `[A-Za-z0-9._-]` an underscore, Windows reserved device stems such as `AUX`/`NUL`/`COM1` prefixed with `_`, the content-addressed file name as a fallback) and never from the request; `Content-Length` is the file's own byte count, the content type comes from its suffix (a PE suffix gets the portable-executable type, everything else `application/octet-stream`), and `Cache-Control` marks the answer immutable because the bytes are content-addressed. An unknown id is 404 `binary not found`; a row whose file is gone is 404 `binary not on disk` naming the path (the engine routes answer 400 for that condition, but a download is of a representation that is gone). `reportal download <binary-id> [--output PATH] [--force]` writes the same bytes to an explicit path in bounded chunks, and the Binaries view carries a per-row Download link. The hosted portal's per-row download is the comparison; this is a local file write with no upload or storage backend. |
| Binary export (symbols rewritten in) | Implemented | local (ELF `.symtab`/`.dynsym`, PE export table) | The hosted spec (v4.54.0) pairs `POST` and `GET /v3/analyses/{analysis_id}/binary-export`: the POST queues an export of "the binary with its current symbols rewritten in" and the GET streams the result named `binary.sym.elf`. reportal's pair is synchronous and stores nothing, because the work is one rewrite over an in-memory copy: `GET /api/binaries/<id>/binary-export` streams the stored file as `<stem>.sym<suffix>` with every non-placeholder store name written over the file's own symbol at its address (an ELF name in its `.symtab`/`.dynsym` string table, a PE name in the export table's), only where the new name fits the existing NUL-terminated slot, so the layout never moves; `X-Reportal-Export-Applied` and `X-Reportal-Export-Skipped` carry the counts and `Cache-Control: no-store` reflects names that change. `POST` on the same path answers the report: `file_name`, `format`, the `applied` rows (`{va, old, new}`), the `skipped` rows with `name-too-long`, the exact `applied_count`/`skipped_count`, `without_symbol` (the renames no table carried, which is every one on a stripped image) and a `note`; both row lists cap at `symbols.MAX_EXPORT_ROWS` while the counts stay exact. A placeholder store name never replaces a real symbol, a PE with no export table says so in the note, and a file that is neither format is 400 `unsupported-format` (a malformed table is 400 `symbols-unreadable`); an unknown id is 404 `binary not found` and a gone file 404 `binary not on disk`, the download route's own checks. `reportal binary-export <binary-id> [--output PATH] [--force]`, the destructive `export_binary` MCP tool and the binary detail Export link expose the same; an over-long name is reported, never a relink or a guess. |
...
| FLIRT signature matching | Implemented (optional checkout) | python-flirt + local store | `reportal flirt <binary-id> [--arch TEXT]`, `GET`/`POST /api/binaries/<id>/flirt`, `reportal flirt-refresh` / `POST /api/flirt/sigsets/refresh`, and `GET /api/flirt/sigsets` index compiled `.sig` files under `REPORTAL_FLIRT_SIGS_DIR` (`flirt_sigs.py`), match one binary through `python-flirt` with a process-local matcher cache, and store the reading as the `flirt` scan plus a `flirt_scan` cache row keyed by `(binary_sha256, sigset_key, arch)`. `GET` is stored-only (404 `no-flirt-scan` before the first POST). `reportal flirt-apply <function-id> --name TEXT` / `POST .../flirt/apply` renames one matched symbol and refuses a person-authored name (`manual-name`). Without the env checkout every FLIRT route answers 400 `no-signature-dir`. This is separate from the engine FLIRT hits inside auto-unstrip and triage: those come from rebrew; this catalog is reportal's own signature sets and never runs the engine. |
...
| Function explain agents (Agents: crypto, execution, filesystem, networking) | Implemented (deterministic text match) | local store (stored decompilation text) | `reportal function-explain <function-id> <domain> [--all]`, `GET /api/functions/<id>/explain/<domain>` and the read-only `explain_function` MCP tool match one domain against the imports and quoted literals the function's own stored decompilation mentions, through `behavior.classify` and the same rule table the binary-level scan uses (`function_extras.function_explain`). `crypto` is a rule table of its own (`behavior.CRYPTO_RULES`: symmetric cipher, hash, key management and CSPRNG APIs, with AES/SHA/RSA/PEM/PKCS literals), because the binary-level behavior scan has no crypto domain; the other three domains are `behavior.BEHAVIOR_RULES`. The payload reports `findings` (the matched evidence name, the rule label, `import` or `string`, confidence and count), the exact `count` and `by_confidence`, the input counts, `has_decompilation` and a `derivation` note stating that a text match is not a model narrative; an unknown domain is 404 `domain not found`, an unknown function 404 `function not found`. The function detail Explain panel selects a domain. The hosted agents are models; this half is deterministic, needs no endpoint, makes no engine call and never executes the binary, and the narrative half is deliberately absent. |
...
| Agent conversations | Implemented (optional) | local store + external LLM (OpenAI-compatible) | `reportal chat-new --function <id> \| --binary <id>` and `POST /api/conversations` open a chat scoped to one stored function or binary; `reportal chat <conversation-id> "message"` and `POST /api/conversations/<id>/messages` append the turn and call the configured chat-completions endpoint. The request is a fixed system prompt plus a context block assembled from stored local data only (the function row with its stored disassembly and decompilation, or the binary row with its stored triage summary and capability scan) and the last `HISTORY_TURN_LIMIT` turns. A plain `POST .../messages` turn runs no engine and calls no tool. Agent runs (`POST /api/conversations/<id>/runs`, `reportal conversation-run`, the destructive `run_conversation_agent` MCP tool) add a local MCP tool loop with a confirmation gate on destructive calls. Conversations and their messages live in the `conversations` and `messages` tables (`GET /api/conversations?scope_kind=&scope_id=`, `GET`/`DELETE /api/conversations/<id>`), the SPA Conversations view lists, creates, opens, sends to and deletes threads, and the function and binary detail views carry a Chat about this action. A message without a configured endpoint answers 503 `llm-unavailable`; the whole feature is off by default. Plain turns answer from stored context only; agent runs may call local MCP tools under the confirmation gate. |
| Integrations inventory | Implemented | local registries | The hosted portal's Integrations page lists the plugins, SDKs and the MCP endpoint it offers. reportal answers the same question about itself: `reportal integrations` and `GET /api/integrations` read the nine plugin seams a plugin enters through (`reportal.components`, `reportal.auto_workers`, `reportal.graph_backends`, `reportal.effect_handlers`, `reportal.mcp_tools`, `reportal.external_sources`, `reportal.models`, `reportal.sandbox_runners`, `reportal.debug_backends`) and report, per seam, the entry-point group, the in-tree module declaring the built-ins and one row per part the registry holds with the fields that seam exposes (a component's origin, requires and provides; a backend's availability and whether it can be queried; a worker's write plan; a handler's descriptor kind and whether it is built-in; a tool's destructive annotation). The read-only `list_integrations` MCP tool and the SPA Integrations view render the same payload. Only the component registry tracks where a part was declared, so an empty origin means the registry does not record one, never that the part is built-in. This is reportal's own seam set, not the hosted platform's plugin catalogue. |
| MCP tool server | Implemented | local + rebrew + optional LLM | The hosted portal's MCP server (`https://api.reveng.ai/mcp/`, Streamable HTTP with `Authorization: Bearer` and an `mcp-session-id`) is matched locally by `reportal mcp`, an stdio server on the official `mcp` SDK (newline-delimited JSON-RPC 2.0 on stdin/stdout) that serves `initialize`, `notifications/initialized`, `tools/list` and `tools/call` at a protocol version the MCP SDK negotiates and reports `serverInfo` `{"name", "title", "version"}`. It exposes reportal's capabilities as the plugin tools declared in `src/reportal/mcp_tools.py` (catalogued in [MCP_TOOLS.md](MCP_TOOLS.md); counts pinned by `tests/test_mcp.py`); a third party registers through the `reportal.mcp_tools` entry-point group. Destructive tools carry `destructiveHint: true` and cover the same engine runs and store mutations the HTTP routes do; handlers call reportal's internal functions directly and never make an HTTP request back into reportal. The read tools keep the stored-only routes stored-only. Stdio (`reportal mcp`) is the local pipe. Streamable HTTP is `POST /mcp` (JSON replies) and `GET /mcp` (SSE session stream) on the same FastAPI process, gated by the portal bearer when auth is on (write, because the registry mixes readers and writers). `Last-Event-ID` resumes from an in-process event store (dropped on restart). `logging/setLevel` sets the reportal stderr logger; `notifications/message` is not pushed. |
...
| Live debug session | Implemented (off by default) | lldb-dap and gdb (external backends) | `reportal debug-session <binary-id> [--timeout N] [--breakpoint VA]... [--qemu ARCH] [--report\|--status\|--coverage]`, `reportal debug-proposals <binary-id>`, `reportal debug-apply <function-id> [--name TEXT]`, `POST`/`GET /api/binaries/<id>/debug-session`, `GET .../debug-session/status`, the stored-only `GET .../debug-coverage` and `GET .../debug-proposals`, `POST /api/functions/<id>/debug-apply`, plus the `run_debug_session` / `get_debug_session` / `get_debug_status` / `get_debug_coverage` / `get_debug_proposals` / `apply_debug_proposal` MCP tools, the `debug` job kind, and the binary detail Debug Session panel (with observed coverage). The probe launches the sample stopped (lldb-dap over DAP, gdb over MI, or `qemu-<arch>` stubs through `target remote`), reads the thread list, one frame, the general registers and a bounded memory window, then disconnects; breakpoint addresses are recorded while the probe stops at entry or main only. Nothing is stepped, continued or written. Four guards hold before any session starts: the workspace opts in (`REPORTAL_DEBUG=enabled` or `[debug] enabled = true`, else 403 `debug-disabled`), a backend is installed (else 503 `debug-unavailable`), the row has a file on disk, and the bounds are inside the caps (`timeout` 1-120s, at most 64 breakpoints; outside them 400 `invalid-debug`). The transcript is stored as the `debug-session` scan in one journaled action (revert removes the record), rendered into a binary-scoped knowledge digest the graph rebuild links through VA mentions, joined to the stored functions as observed coverage, and read as rename proposals (source `debug`, person-authored names never touched, explicit apply only). The hosted portal has no live debugger; this is reportal's own tier, and `docs/THREAT_MODEL.md` states the boundary-7 residual in full. |
...
- **A reconstructed re-run from the stored rows, partly.** A scan records the
  ...
  `POST /api/analyses/<id>/requeue` moves the lifecycle row back to `pending`
  and queues a job for each stored scan that has a job kind
  (`jobs.queue_stored_scans`), using the recorded params the kind takes. A scan
  with no job kind (`unpack`, which creates a new binary) stays a per-scan POST:
  those producers need the engine, the project context or the LLM, and a silent
  replay would differ.
...
reportal's own surface for the comparison is its FastAPI OpenAPI schema (254
paths, 353 operations) plus the MCP tool registry documented in [MCP_TOOLS.md](MCP_TOOLS.md).  Every row below is
...
operation in the registry: the queued scans, the engine report, the PDF,
...
- `GET /api/iam/me` and `.../permissions` answer the hosted `iam/me` pair: the
  ...
  auth off the caller is the local operator and the answer says so.  Named extra
  API keys (`GET|POST /api/iam/keys`, `PATCH|DELETE /api/iam/keys/<id>`,
  `reportal api-keys`/`api-key-add`/`api-key-rename`/`api-key-rm`, MCP
  `list_api_keys` / `create_api_key` / `rename_api_key` / `revoke_api_key`) sit
  beside the login token, count toward
  the organisation plan's `max_api_keys` (the login token is one), and never
  return a digest; `last_used_at` is stamped on a successful named-key
  authenticate and stays empty until then; a `read_only` named key authenticates
  as the user but HTTP writes and `/mcp` are 403; the login token is never
  read-only; the login token stamps
  `users.last_used_at` the same way; a used-up cap is 402 `api-key-limit`.
...
- the gate itself: the `server._reportal_headers` middleware calls
  ...
  armed only by a truthy `REPORTAL_AUTH` (`1`, `true`, `yes`, `on`, `enabled`,
  or `required`) or `[auth] required = true`, so a
...
- the per-object enforcement that scope needs: `server._scoped_object` resolves
  ...
  permitted`.
- `reportal users`/`user-add`/`signup`/`user-token`/`user-edit`/`user-rm`,
  `reportal teams`/`team-add`/`team-rm`/`team-member`/`team-invite`/
  `team-invites`/`team-invite-rm`/`team-join`/`binary-scope`/`collection-scope`, the
  `list_users`, `signup`, `add_user`, `rotate_user_token`, `update_user`,
  `delete_user`, `list_teams`, `list_team_invites`, `create_team`,
  `delete_team`, `add_team_member`, `remove_team_member`,
  `create_team_invite`, `revoke_team_invite`, `join_team`, `set_binary_scope` and
  `set_collection_scope` MCP tools and
  the SPA (the Users view with its identity block, the browser's bearer-token
  field, the user table, the Teams panel and the Invites panel, plus the
  Binaries table's per-row scope select) expose the same.
  `docs/THREAT_MODEL.md` records the moved boundary and its residual risks.
...
`get_external_status` and `run_external_source` MCP tools (see
[MCP_TOOLS.md](MCP_TOOLS.md)) and the SPA's External view expose the same, and a
...
- the per-function explain agents (`POST /v3/functions/{id}/crypto-explain:run`,
  ...
  each with its result read under `/v3/operations/...`): `GET
  /api/functions/<id>/explain/<domain>` runs the same derivation as the
  capabilities read through `behavior.classify` and the binary-level behavior
  rule table, with a `crypto` rule table of its own for the domain the
  binary-level scan does not carry.  The hosted agents answer a narrative
  (purpose, trigger, what executes); this answers the evidence a narrative would
  cite and states that in `derivation`, so nothing here needs an endpoint or an
  engine call.
...
- nearest functions for one function or a pasted body (the hosted ANN search,
  `reait -n`): `POST /api/functions/<id>/similar` and `POST
  /api/functions/similar` rank every function with a cached listing through the
  LSH index above a similarity of 80 and pairwise below, with the same hits
  either way, and record nothing.
...
`reportal indirect-calls`, `function-capabilities`, `function-explain`,
`function-strings`,
...
`explain_function`, `get_function_strings`, `list_analysis_strings`,
`list_function_edges`,
...
`delete_function_edge` and `canonicalize_function_names` MCP tools (see
[MCP_TOOLS.md](MCP_TOOLS.md)); and the function detail's indirect-call-site,
capabilities, explain, strings, callees and canonical-name panels plus the
analysis log
...
`get_data_type_functions` MCP tools (see [MCP_TOOLS.md](MCP_TOOLS.md) for the live registry);
...
`list_conversation_runs` and `get_conversation_run` (see
[MCP_TOOLS.md](MCP_TOOLS.md)); and the conversation detail's Agent run panel (the question
...
| 2 | **Cross-architecture symbol matching** | "We will be releasing a cross-architecture model to match symbols between architectures in Q3 2026" (`reveng.ai`) | Done: a match run scores stored listings across the corpus without an architecture model; every recorded row stores the ISA pair (`source_arch`, `candidate_arch`) and the Matches view flags a cross-architecture pair |
| 3 | **Ventris architecture and language coverage** | "we are also planning to release a version of Ventris in the coming months that provides wider architecture support"; "expanding language coverage to Rust and Go, and supporting additional architectures such as MIPS, PowerPC, and RISC-V" (`reveng.ai/blog/introducing-wilbert-and-ventris`) | Done: a binary row records `arch` (ISA), `language` (stamped from the filetype runtime match, overwritten by Go buildinfo) and `compiler` (stamped from the filetype toolchain match). The register filters on `?language=` / `?compiler=` and a coverage gap is an empty column, not a silent miss |
| 4 | **MCP destructive annotations on every writer** | 27 of the hosted server's 36 tools mutate state but only 9 carry a destructive marker (`docs.reveng.ai/mcp`) | Done: every local tool carries `readOnlyHint` or `destructiveHint`, and `tests/test_mcp.py` pins the split |
...
| 9 | **The documented-but-missing client flags** | `reait`'s README documents `-n` (ANN search), `--symbol`, `--start-vaddr`, `--image-base`, `-C` (open-source component identification) and "YARA++" signatures; none exist in its argparse parser, and `api.py` never calls its ANN or SBOM functions (`github.com/RevEngAI/reait`) | Done: ANN search is the similar-functions query (`POST /api/functions/similar`, `reportal similar`) over the LSH index; the other equivalents are answered by the typed search, the families store, the data-type model and the remediation rules, and the SBOM is `library.py` plus `GET /api/binaries/<id>/sbom`, which renders the stored library identification as CycloneDX, SPDX or CSV. |
| 10 | **Public limits** | no pricing, tier or rate-limit page exists (`reveng.ai/pricing` 404s; only `TOO_MANY_REQUESTS` with `Retry-After`) | Done: `GET /api/config` publishes every cap in force, `docs/API.md` names each one, and HTTP 429 `rate-limited` carries `Retry-After` |
