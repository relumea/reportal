...
| Live debug session | Implemented (off by default) | lldb-dap (external backend) | `reportal debug-session <binary-id> [--timeout N] [--breakpoint VA]... [--report\|--status]`, `POST`/`GET /api/binaries/<id>/debug-session` and `GET .../debug-session/status`, plus the `run_debug_session` / `get_debug_session` / `get_debug_status` MCP tools and the binary detail Debug Session panel. Slice 1 of `docs/LIVE_DEBUGGING.md`: the probe launches the sample stopped under lldb-dap, waits for the entry stop, reads the thread list, one stack frame, the general-purpose registers and a bounded window at the instruction pointer, then disconnects. Nothing is stepped, continued or written. Four guards hold before any session starts: the workspace opts in (`REPORTAL_DEBUG=enabled` or `[debug] enabled = true`, else 403 `debug-disabled`), a backend is installed (else 503 `debug-unavailable`), the row has a file on disk, and the bounds are inside the caps (`timeout` 1-120s, at most 64 breakpoints; outside them 400 `invalid-debug`). The transcript is stored as the `debug-session` scan in one journaled action (revert removes the record). The hosted portal has no live debugger; this is reportal's own tier, and `docs/THREAT_MODEL.md` states the boundary-7 residual in full. |
