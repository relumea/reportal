...
paths, 314 operations) plus the MCP tool registry (244 tools).  Every row below is
...
`get_data_type_functions` MCP tools (244 tools: 115 read-only, 129 destructive);
...
| 4 | **MCP destructive annotations on every writer** | 27 of the hosted server's 36 tools mutate state but only 9 carry a destructive marker (`docs.reveng.ai/mcp`) | Done: all 244 local tools carry `readOnlyHint` or `destructiveHint`, and a test pins the split |
