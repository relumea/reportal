...
reportal's own surface for the comparison is its FastAPI schema (240
paths, 339 operations) plus the MCP tool registry (262 tools).  Every row below is
...
`get_data_type_functions` MCP tools (262 tools: 122 read-only, 140 destructive);
...
| 4 | **MCP destructive annotations on every writer** | 27 of the hosted server's 36 tools mutate state but only 9 carry a destructive marker (`docs.reveng.ai/mcp`) | Done: all 256 local tools carry `readOnlyHint` or `destructiveHint`, and a test pins the split |
