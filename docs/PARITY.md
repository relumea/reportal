...
paths, 289 operations) plus the MCP tool registry (225 tools).  Every row below is
...
`get_data_type_functions` MCP tools (225 tools: 105 read-only, 120 destructive);
...
`list_conversation_runs` and `get_conversation_run` (225 tools: 105 read-only,
