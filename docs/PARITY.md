...
reportal's own surface for the comparison is its FastAPI schema (222
paths, 308 operations) plus the MCP tool registry (238 tools).  Every row below is
...
`get_data_type_functions` MCP tools (238 tools: 112 read-only, 126 destructive);
...
| 4 | **MCP destructive annotations on every writer** | 27 of the hosted server's 36 tools mutate state but only 9 carry a destructive marker (`docs.reveng.ai/mcp`) | Done: all 238 local tools carry `readOnlyHint` or `destructiveHint`, and a test pins the split |
...
Rows 8 and 11 stay open on purpose, each with the reason stated in its row,
and neither is a capability the hosted portal ships, so neither is a parity gap
and neither is tracked in `docs/TODO.md` (no crawl entry asked for one).  Row 1
