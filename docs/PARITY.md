...
paths, 338 operations) plus the MCP tool registry (261 tools).  Every row below is
...
- `GET /api/iam/me` and `.../permissions` answer the hosted `iam/me` pair: the
  ...
  API keys (`GET|POST /api/iam/keys`, `DELETE /api/iam/keys/<id>`,
  `reportal api-keys`/`api-key-add`/`api-key-rm`, MCP `list_api_keys` /
  `create_api_key` / `revoke_api_key`) sit beside the login token, count toward
  ...
  authenticate and stays empty until then; the login token stamps
...
`get_data_type_functions` MCP tools (261 tools: 122 read-only, 139 destructive);
