...
reportal's own surface for the comparison is its FastAPI schema (224
paths, 311 operations) plus the MCP tool registry (241 tools).  Every row below is
...
`get_data_type_functions` MCP tools (241 tools: 114 read-only, 127 destructive);
...
| 4 | **MCP destructive annotations on every writer** | 27 of the hosted server's 36 tools mutate state but only 9 carry a destructive marker (`docs.reveng.ai/mcp`) | Done: all 241 local tools carry `readOnlyHint` or `destructiveHint`, and a test pins the split |
