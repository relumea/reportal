...
reportal's own surface for the comparison is its FastAPI schema (226
paths, 314 operations) plus the MCP tool registry (246 tools).  Every row below is
...
`get_data_type_functions` MCP tools (246 tools: 115 read-only, 131 destructive);
...
| 4 | **MCP destructive annotations on every writer** | 27 of the hosted server's 36 tools mutate state but only 9 carry a destructive marker (`docs.reveng.ai/mcp`) | Done: all 246 local tools carry `readOnlyHint` or `destructiveHint`, and a test pins the split |
