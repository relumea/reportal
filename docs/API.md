...
| `/api/health` | GET | status, version, database path, row counts, process-local `http` request counters (requests, 4xx/5xx, latency sum and max since start) and `jobs` counters (done, failed, latency sum and max); `dependencies.jobs` is the live queue depth (queued/running) and whether the process pool is on; every `/api` response echoes `X-Request-Id` (accepted when well-formed, otherwise minted); this is the running server's read, and `reportal doctor` is the pre-flight half (the SPA build, a free port, the schema and the auth posture before anything starts) |
