...
| `/api/binaries/<id>/debug-session` | POST | run a read-only debug probe over a stored binary and store the transcript; body `{"timeout": seconds, "breakpoints": [va, ...]}` narrows the caps (400 `invalid-debug` outside them or for a non-integer); 403 `debug-disabled` unless the workspace opted in, 503 `debug-unavailable` without an installed backend, 404 `binary not found`, 400 `binary not on disk`; 201 with the finished session (the backend, the caps, the transcript), one journaled row; a second POST while a session is still `running` returns that live row as 202 without starting another probe |
