...
| `/api/binaries/<id>/flirt/apply` | POST | rename one function to a symbol the signatures matched; body `{"function_id", "name"}` (the name is the caller's, because the matcher reports symbols rather than addresses); 404 `function not found` or unknown binary, 400 `manual-name` when a person authored the current name, `invalid name` for a blank one; journaled as one name change |
...
| `/api/journal` | GET | recorded action-journal entries, newest first, without their descriptor payload; `?limit=` (default `journal.DEFAULT_LIST_LIMIT`, hard cap `journal.MAX_LIST_LIMIT`), optional `?action=` and optional `?actor=` (the identity the server set around the write, `local` while auth is off and empty for a CLI or MCP one); the body echoes what it applied and names the `actors` the journal holds; 400 for a non-integer or non-positive limit; read-only |
