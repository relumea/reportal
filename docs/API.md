...
| `/api/binaries/<id>/decompiler-script` | GET | render the stored renames as a runnable decompiler script: a Ghidra Python script (`?format=ghidra`, the default), an IDA script (`?format=ida`) or a Binary Ninja rename document (`?format=binja`); 400 `invalid format`, 404 unknown binary; stored-only, placeholders are left out |
