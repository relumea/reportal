...
| `/api/binaries/<id>/composition` | GET | stored composition analysis (including `tags` from the matched binaries); 404 `no-scan` without one; stored-only, never runs matching or the engine |
...
| `/api/binaries/<id>/decompiler-script` | GET | render the stored renames as a runnable decompiler script: a Ghidra Python script (`?format=ghidra`, the default), an IDA script (`?format=ida`) or a Binary Ninja rename document (`?format=binja`); 400 `invalid format`, 404 unknown binary, 400 `unsafe name` when a rename holds both quote styles and cannot be quoted into a script; stored-only, placeholders are left out |
