...
| `/api/iam/keys` | GET | the caller's named extra keys, without digests; `used` is login token plus named keys, `limit` is the plan cap (`null` when unlimited); `last_used_at` is empty until the key authenticates; self-service; empty while auth is off |
| `/api/iam/keys` | POST | mint one named extra key; body `{"name"}`; 201 with the key and `token` shown once; journaled; 400 `invalid-api-key`, 402 `api-key-limit`; self-service, needs token auth |
...
| `/api/binaries` | GET | the binaries the caller may see, each with its function and comment counts, stored `format`/`arch` and recovered `language`/`compiler` (empty when unknown); `?search=` matches the name, the SHA-256 (a prefix works) or the operator notes, `?tag=` keeps the binaries carrying that exact tag name, `?format=` one stored format, `?language=` one recovered language, `?compiler=` one recovered toolchain and `?order=` one of `store.BINARY_ORDERS` (`id`, the default, then `newest`, `name`, `name-desc`, `size` and `size-desc`); an unknown order is 400 `invalid order`; the body echoes every filter it applied beside `count` (returned) and `total` (unfiltered, so a filter that matched nothing is distinguishable from an empty register) and names the `formats`, `languages` and `compilers` the register holds |
...
| `/api/binaries/<id>` | GET | one binary, plus `rebrew_project`: the rebrew project its engine-backed reads load, `null` when it was imported without one (the state every 400 `no-engine-context` comes from) |
...
| `/api/binaries/<id>/data-types` | GET | the editable type model with each type's kind, namespace, size, members, enum values, target and padded `as_c` declaration, plus the namespace tree; always answers; optional `kind` (400 `invalid kind` outside `data_types.KINDS`), `namespace` (the path and every descendant, `Binary` for program-defined types, an unknown path an empty list), `search` (a substring of the name, a member name or an enum value name) and `source` (one of `data_types.SOURCE_LABELS`, 400 `invalid source` outside them), `sort` (`name`, the default, or `size`; 400 `invalid sort`) and `direction` (`asc`, the default, or `desc`; 400 `invalid direction`), where a type whose size the model states as zero (an unknown one) sorts last in either direction; the body carries `count` (filtered), `total` (unfiltered), the `sort`/`direction` it applied and `sources` (the count per provenance label over the whole model) |
...
| `/api/functions/<id>/matches` | GET | recorded matches for one function, each with the derived `difference` and `band` fields, the stored `source_arch`/`candidate_arch` pair and `cross_arch` |
...
| `/api/binaries/<id>/matches` | GET | stored-only; the binary's recorded edges with the run settings they were recorded under (`settings`, null outside a run), the derived `difference` and `band` fields, the stored `source_arch`/`candidate_arch` pair, `cross_arch` and the scope `notes` |
...
| `/api/binaries/<id>/auto` | POST | plan an auto run and execute it in a background thread; body `{"worker", "execute", "concurrency", "functions_per_task", "max_attempts", "max_tasks"}`, all optional and every bound validated (400 `invalid params`); answers 202 `{"run_id", "binary_id", "status": "running"}`; `execute` defaults to false; SaaS tenants past quota get 402 `quota-exceeded` |
