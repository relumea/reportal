...
401 `unauthorized`, a role that does not carry the permission the method and
path imply is 403 `forbidden`, and an authenticated HTTP write past 60 in
60 seconds is 429 `rate-limited` with `Retry-After`.  Roles are `viewer` (read), `analyst` (read and
...
|------|--------|-------------|
| `/api/signup` | POST | SaaS self-serve: create a user, organisation and owned team on the free plan; body `{"name"}`; 201 with the user, tenant rows and `token` shown once; no bearer required; 403 `signup-disabled` on the personal profile, 409 `user-exists`, 429 `rate-limited` (with `Retry-After`) past five attempts from one TCP peer in an hour; journaled |
| `/api/iam/me` | GET | who the caller is: `auth` (`open` or `required`), the `user` (including `last_used_at` on the login token), its `role` and the `permissions` that role carries; with auth off the user is null and the permissions are all three, because the caller is the local operator |
| `/api/iam/me/permissions` | GET | the caller's `role`, the `permissions` it carries, the `auth` mode and the closed `roles` list |
| `/api/iam/keys` | GET | the caller's named extra keys, without digests; `used` is login token plus named keys, `limit` is the plan cap (`null` when unlimited); `last_used_at` is empty until the key authenticates; `read_only` is false until minted that way; self-service; empty while auth is off |
| `/api/iam/keys` | POST | mint one named extra key; body `{"name", "read_only"?}`; 201 with the key and `token` shown once; a `read_only` key authenticates as the user but HTTP writes and `/mcp` are 403 `forbidden`; journaled; 400 `invalid-api-key`, 402 `api-key-limit`; self-service, needs token auth |
| `/api/iam/keys/<key_id>` | PATCH | rename one of the caller's named extra keys; body `{"name"}`; journaled; the token is unchanged; 400 `invalid-api-key`, 404 `api-key-not-found` when unknown or owned by someone else |
| `/api/iam/keys/<key_id>` | DELETE | revoke one of the caller's named extra keys; journaled; 404 `api-key-not-found` when unknown or owned by someone else; the login token is rotated, not deleted here |
| `/api/users/activity` | GET | the activity feed, derived rather than stored: one item per journaled action (its `actor`, `action`, newest `description`, `at`, `status`, `entries`) plus one per analysis-log entry, newest first (each item carries `seq`, the source row id that breaks a same-second tie, shared with the notifications feed), with `count`, the true `total`, `latest` and the `actors` that appear; `?actor=` (an empty value means the writes no request made), `?since=` (inclusive ISO), `?limit=` (default `activity.DEFAULT_ACTIVITY_LIMIT`, cap `MAX_ACTIVITY_LIMIT`) and `?sources=action,log`; a bad value is 400; self-service, so any authenticated caller reads it |
...
| `/api/users` | GET | every user with its `role`, `has_token`, `disabled`, `created_at` and `last_used_at` (empty until the login token authenticates); the token digest is never part of an answer; admin only when auth is on |
...
| `/api/teams` | GET | every team with its `member_count` and `member_ids` (the user ids already on it, one grouped query); readable by any authenticated caller |
...
| `/api/teams/<id>/invites` | POST | mint one single-use invite code (returned once, expires after seven days); team owner or admin only; journaled |
| `/api/teams/<id>/invites` | GET | list a team's invites without code digests (`expires_at`, `expired`); team owner or admin only |
| `/api/teams/<id>/invites/<invite_id>` | DELETE | revoke one unused invite; journaled; 404 `invite-not-found` when unknown or on another team, 410 `invite-used` when already redeemed; team owner or admin only |
| `/api/teams/join` | POST | redeem one invite code and join its team; body `{"code"}`; needs token auth; 404 `invite-not-found`, 410 `invite-used` or `invite-expired`; journaled |
...
| `/api/teams/<id>/organisation` | PUT | move a team into an organisation or out of every one; body `{"organisation_id": <id>\|null}`; tenant admin only (auth off is the local operator); 403 `forbidden` for a non-admin, 404 `organisation-not-found` for an unknown id, 400 `invalid-organisation` for a non-integer; journaled |
...
| `/api/organisations/<id>/billing/portal` | POST | open the provider's self-service portal for the organisation; answers `{"provider", "url"}`; 404 `organisation-not-found` |
| `/api/billing/manual/confirm` | POST | confirm a manual (development) checkout: body `{"token", "organisation_id"}`; grants the plan once; 400 `invalid body` |
| `/api/organisations/<id>/billing/cancel-manual` | POST | cancel a manual subscription, dropping the org to the fallback plan; 404 `organisation-not-found` |
| `/api/billing/webhook` | POST | apply a provider webhook (verify, claim, mirror, entitle); a redelivery answers `duplicate` without touching anything and an event naming no resolvable tenant is recorded unapplied; 400 `invalid signature` |
| `/api/organisations/<id>/billing/sync` | POST | re-read the org's subscription from the provider and mirror it; webhooks are the primary path and this only mirrors, so it cannot grant entitlement on its own; rate limited per organisation (429 with `Retry-After` past the cap) |
...
| `/api/health` | GET | status (`ok` or `degraded` when `failures` is non-empty), version, database path, row counts, process-local `http` request counters (requests, 4xx/5xx, latency sum and max since start) and `jobs` counters (done, failed, latency sum and max); `dependencies.jobs` is the live queue depth (queued/running) and whether the process pool is on; a database that is not writable or that fails to open is named in `failures` while the HTTP status stays 200; every `/api` response echoes `X-Request-Id` (accepted when well-formed, otherwise minted); this is the running server's read, and `reportal doctor` is the pre-flight half (the SPA build, a free port, the schema and the auth posture before anything starts) |
| `/api/doctor` | GET | the pre-flight half over HTTP: `doctor.report` with an optional `?port=` (default 8002, 0 or blank skips the bind probe); every check is a read, nothing is written and no database is created, and the answer is 200 with the same `ok`/`degraded` vocabulary either way; 400 `port must be an integer` |
| `/mcp` | POST | Streamable HTTP MCP: the same registry as `reportal mcp` ([MCP_TOOLS.md](MCP_TOOLS.md)), JSON-RPC 2.0 over one POST (`initialize`, `tools/list`, `tools/call`). Auth is the same bearer as `/api` (loopback operator while auth is off); a viewer is 403 because the registry mixes readers and writers. JSON replies. 503 `mcp-unavailable` if the process lifespan has not started the session manager |
| `/mcp` | GET | SSE session stream (`Accept: text/event-stream`); `Last-Event-ID` resumes from the in-process event store. 406 without that Accept. Same bearer as POST. 503 `mcp-unavailable` if the session manager is not running |
| `/mcp` | DELETE | end one Streamable HTTP MCP session; same bearer as POST. 503 `mcp-unavailable` if the session manager is not running |
...
| `/api/binaries` | GET | the binaries the caller may see, each with its function and comment counts, stored `format`/`arch` and recovered `language`/`compiler` (empty when unknown); `?search=` matches the name, the SHA-256 (a prefix works) or the operator notes, `?tag=` keeps the binaries carrying that exact tag name, `?format=` one effective format (a hand-set override first), `?language=` one recovered language, `?compiler=` one recovered toolchain and `?order=` one of `store.BINARY_ORDERS` (`id`, the default, then `newest`, `name`, `name-desc`, `size` and `size-desc`); an unknown order is 400 `invalid order`; `?summary=true` projects each row down to its `id` and `name`, which is what a picker offering the whole register reads, and anything but a boolean is 400 `summary must be a boolean`; `?limit=` cuts one page bounded by `store.MAX_BINARY_LIMIT` and `?offset=` moves into the register, so a large register is read a page at a time, outside that bound is 400 `invalid limit` and a negative offset is 400 `invalid offset`, and a caller naming no limit still reads the whole register; the body echoes every filter it applied beside `count` (this page), `matched` (the rows the filter kept, which a paging control counts against) and `total` (unfiltered, so a filter that matched nothing is distinguishable from an empty register) and names the `formats`, `languages` and `compilers` the register holds |
| `/api/binaries/<id>` | PATCH | set a binary's display name, operator notes and/or format/ISA override; body `{"name"}`, `{"notes"}` and/or `{"format_override", "arch_override"}` (`name` non-empty after strip; empty `notes` clears the note; past 2000 chars is 400; each override names one of the upload vocabularies, empty clears it so detection stands again); journaled; 400 `invalid binary` when no field, blank name, oversized notes or an unknown override; 404 `binary not found`; sha256 dedupe is unchanged |
| `/api/binaries` | POST | upload one or many binaries; `multipart/form-data` with repeated `file` parts and an optional JSON `files` field (entry *i* describing part *i*: `name`, `tags`, `collection_ids`, `format`/`arch`/`compiler`, and the `visibility`/`team_id` scope the binary should carry, where a `team_id` alone means team visibility and a named team the caller is not in is 403 `not-a-team-member`); one `file` part with no `files` field is the single-file upload whose response is unchanged (`duplicate` flag, deduped by sha256); a batch answers `{"files": [{"file", "binary_id", "duplicate", "tags", "collections", "visibility", "owner_team_id", "error"}], "count", "duplicates", "errors"}` with the same refusal vocabulary per entry (a duplicate the caller may not re-scope is `scope-forbidden` on its entry), and the whole request is one journal action; each binary the request creates queues the `analyse` job (function discovery into `projects/<sha256>` and the import) and carries it as `analysis_job` (`id`, `status`), `null` for a duplicate or when the queue is full |
| `/api/binaries/<id>/extract` | POST | unpack a stored archive and register its members by content hash into one collection (the body's `collection_id`, else a collection named after the archive); each supported member is validated first (traversal, absolute name, symlink/hardlink, device/FIFO, per-member/per-total/per-ratio bomb caps) and reported with its id or its skip reason, and the request is one journal action; 201, 404 unknown binary or collection, 400 `binary not on disk`, 400 `unsupported-format`, `external-tool-required` (`.rar`/`.7z` name the tool), `password-required`/`bad-password` |
| `/api/binaries/<id>` | GET | one binary, including its stored `created_at`, plus `rebrew_project`: the rebrew project its engine-backed reads load, `null` when it was imported without one (the state every 400 `no-engine-context` comes from) |
...
| `/api/binaries/<id>/binary-export` | GET | stream the stored binary with its current names rewritten into its own symbol tables (an ELF name in its `.symtab`/`.dynsym` string table, a PE name in the export table's) as `<stem>.sym<suffix>`; only a non-placeholder stored name whose new value fits the existing NUL-terminated slot is written, so the layout never moves; `X-Reportal-Export-Applied` and `X-Reportal-Export-Skipped` carry the counts, `Cache-Control: no-store` because the bytes depend on names that change; 400 `unsupported-format` for a file that is neither ELF nor PE, 400 `symbols-unreadable` for a malformed table, 404 `binary not found`/`binary not on disk` |
| `/api/binaries/<id>/binary-export` | POST | the same export's report without the bytes: `file_name`, `format`, `applied` (`{va, old, new}`), `skipped` (`{va, old, new, reason}`, `name-too-long`), the exact `applied_count`/`skipped_count`, the renames the file's tables had no entry for (`without_symbol`) and a `note`; both rows lists cap at `symbols.MAX_EXPORT_ROWS` while the counts stay exact; synchronous and stored nowhere, because a rewrite over an in-memory copy is sub-second; same refusals as the GET |
| `/api/binaries/<id>/functions` | GET | functions of one binary, filtered and sorted from the query string: `name_source` (one of `composition.NAME_SOURCE_LABELS`), `capability` (one of `capabilities.CAPABILITIES`), `min_size`/`max_size` (inclusive byte bounds, at most `api.MAX_FUNCTION_SIZE`), `string` (a literal the stored decompilation carries), `match` (`matched`/`unmatched` over the stored `matches` table), `name` (a case-insensitive substring of the function's name, with LIKE wildcards escaped), `va` (one exact address, decimal or `0x` hex; 400 `invalid va` otherwise), `refers_to` (an address; the one engine-backed filter, keeping the stored functions whose byte range contains one of the address's cross-references, resolved through the same `rebrew xrefs` call the xrefs route makes; 400 `invalid refers_to` for a non-integer, 400 `no-engine-context` without a project, 503 without an engine, 500 `engine-error`), `sort` (`va`/`size`/`name`/`status`) and `order` (`asc`/`desc`); an unknown value is 400; ties break on the function id; `limit` (1 to `store.MAX_FUNCTION_LIMIT`, else 400 `invalid limit`) and `offset` (non-negative, else 400 `invalid offset`) page the result, pushed into SQLite unless `refers_to`/`name_source`/`capability` filter after the query; the body carries `count` (returned), `matched` (kept by the filters, before the page) and `total` (the binary's whole function set) |
...
| `/api/binaries/<id>/match` | POST | run local function matching under the Match Settings scope: `{"min_similarity": 80.0, "min_confidence": 0.0, "include_self": true, "top": 10, "platforms": [], "architectures": [], "binary_ids": [], "collection_ids": [], "name_sources": []}` (name sources are the portal display labels, matched case-insensitively), each validated against its bound or closed vocabulary (400 otherwise); returns the summary plus the recorded `settings` and the scope `notes` |
...
| `/api/binaries/<id>/composition` | GET | stored composition analysis (including each rollup row's `labels`: the matched binary's tags, its visible collection membership and its known families, plus the `attribution` rollup of those labels and the `tags` counts); 404 `no-scan` without one; stored-only, never runs matching or the engine |
...
| `/api/binaries/<id>/report/pdf` | POST | render a text-only PDF summary from the stored scans (family detection, related binaries and composition among them) and per-function AI summaries into `<workspace>/reports/<id>/report.pdf`; returns `{"path", "bytes", "pages", "download_url"}`; stored-only, never runs the engine |
...
| `/api/data-types/<id>/members/<member>/move` | POST | move a member to another position, recomputing the layout offsets; body `{"to_index"}` (past the end clamps to last); 400 `invalid index` for a non-integer target |
...
| `/api/functions/<id>/signature/history` | GET | a function's signature-edit history, newest first, each row carrying the state the edit replaced plus that state rendered as a `prototype` (null for the row that created the signature), its `actor_name` (the display name of `actor_user_id`, null when that user row is gone) and a relative `age`; always answers (an empty list for a row predating the history table) |
...
| `/api/binaries/<id>/attack-surface` | GET | the attack surface composed from the stored scans: the network-reachable entries (protocols, the networking capability, threat URL/domain/IPv4 IOCs), the local input handlers (filesystem/execution behavior findings, file-io/registry/process capabilities) and the crypto usage, each with its evidence and source scan; `sources` names every input with the command that fills it in; 404 `no-scan` when no source scan is stored; stored-only, never runs the engine |
| `/api/binaries/<id>/function-rollup` | GET | the binary's function totals counted in SQLite: `total`, `matched` (a status in `store.MATCHED_STATUSES`) and `by_status` (count per stored status, ordered by descending count); what a summary view shows without reading the function rows; 404 `binary not found`; stored-only, never runs the engine |
| `/api/binaries/<id>/section-coverage` | GET | per-section byte coverage over the binary's stored functions and stored `pe-info` sections: each section's `covered`/`size`/`coverage_pct` and the totals, with overlapping functions unioned and a straddling function clipped; 404 `no-scan` without a stored `pe-info` scan, `null` percentages with a zero-function note when nothing is stored; stored-only, never runs the engine |
...
| `/api/binaries/<id>/gobuildinfo` | GET | stored Go build information (compiler version, module path, dependency pins, build settings); 404 `no-scan` without one; stored-only, never runs the engine |
| `/api/binaries/<id>/gobuildinfo` | POST | scan the stored file for the `go.buildinfo` magic and store the version, module, dependency pins and settings; 404 unknown binary, 400 `not-go` for a non-Go binary; no engine, no project context |
...
| `/api/binaries/<id>/exploitability` | GET | rank the stored security findings by reachability: reachable when another stored function's decompilation mentions its function, with a network-adjacency flag; severity, then reachability, then function and rule; 404 `no-scan` without a stored security scan; stored-only, never runs the engine |
...
| `/api/binaries/<id>/sbom` | GET | render the stored library reading plus the stored Go dependency pins as a component list: `?format=cyclonedx` (the default), `spdx` or `csv`; JSON for the two schema shapes and text for the CSV; Go rows carry the pinned version and a `pkg:golang` purl; 400 `invalid format`, 404 unknown binary; stored-only, it never runs the engine again |
...
| `/api/binaries/<id>/unstrip/apply` | POST | apply one stored proposal by `function_id`, recording the rename with source `unstrip`; body `{"function_id": ..., "name": ...}` (`name` optional); 404 `function not found` when the id is unknown or not of this binary, 400 `invalid name` for a non-string or blank override |
| `/api/flirt/sigsets` | GET | the indexed FLIRT signature catalog, newest first within an architecture: one row per `.sig` file with its `arch`, `family`, `source`, content hash, size and pattern count; `?arch=` narrows it; `sigs_dir` and `env` name where the checkout is expected |
| `/api/flirt/sigsets/refresh` | POST | index every `.sig` under the directory `REPORTAL_FLIRT_SIGS_DIR` names and nothing else: one row per file, an unchanged hash left alone, a changed one updated in place keeping its `enabled` flag; answers `added`, `updated`, `unchanged` and `pruned`; 400 `no-signature-dir` when the install configured none |
| `/api/binaries/<id>/flirt` | POST | match one binary against the enabled signatures for its architecture and store the reading: the matched symbols grouped by library, the `sigset_key` they were found under and whether it came from the cached scan of the same bytes; body `{"arch"?}` overrides the stored fingerprint's; 404 unknown binary, 400 `no-signature-dir` or `invalid arch`; journaled, and queued as the job kind `flirt` |
| `/api/binaries/<id>/flirt` | GET | the stored signature reading of one binary, without re-matching it; 404 `no-flirt-scan` before the first POST, 404 unknown binary |
| `/api/binaries/<id>/flirt/apply` | POST | rename one function to a symbol the signatures matched; body `{"function_id", "name"}` (the name is the caller's, because the matcher reports symbols rather than addresses); 404 `function not found` when unknown, of another binary, or for an unknown binary, 400 `manual-name` when a person authored the current name, `invalid name` for a blank one; journaled as one name change |
...
| `/api/functions/<id>/history` | GET | rename history, newest first, each row with its `actor_name` (the display name of `actor_user_id`, null when that user row is gone) and a relative `age` |
| `/api/functions/<id>/history/<history_id>/revert` | POST | revert one history row; returns the restored name |
| `/api/functions/<id>/similar` | POST | the same ranking for one stored function's listing (`disasm_cache`, else the engine through its rebrew context); optional body `{"min_similarity", "limit"}`; the function is never its own hit; 404 `function not found`, 400 `no-engine-context` with no listing and no context, 400 `invalid-similar-query` for any other field or an out-of-range value |
| `/api/functions/<id>/matches` | GET | recorded matches for one function, each with the derived `difference` and `band` fields, the stored `source_arch`/`candidate_arch` pair, `cross_arch`, and the candidate's stored `candidate_prototype` (null when none) |
...
| `/api/binaries/<id>/matches` | GET | stored-only; the binary's recorded edges with the run settings they were recorded under (`settings`, null outside a run), the derived `difference` and `band` fields, the stored `source_arch`/`candidate_arch` pair, `cross_arch`, the source function's `source_name_source`, the candidate's owning binary (`candidate_binary_id` / `candidate_binary_name`) and the scope `notes` |
...
| `/api/search` | GET | the typed search; `?q=`, `?kind=all\|sha256\|binary\|collection\|tag`, `?limit=`, and `?regex=true` to match the query as a bounded, cached regular expression (400 `invalid regex` for one that does not compile, and for `kind=sha256`, which is a literal hash prefix by definition; 400 `regex must be a boolean` outside the shared true/false vocabulary). With `kind=all` the query may carry an inline prefix (`tag:`, `binary:`/`binary_name:`, `collection:`/`collection_name:`, `sha256:`/`sha_256_hash:`/`hash:`) selecting the kind for that query; an explicit `kind` wins, an unknown prefix stays a literal. A literal query of hex digits (`0x` optional) also matches the function at that VA in every binary (`match: "va"`, ranked first); function rows carry `binary_name` |
...
| `/api/functions/matches` | GET | the recorded match rows of many functions in one read, each with the derived `difference` and `band`, the stored `source_arch`/`candidate_arch` pair and `cross_arch`; runs no scoring and no engine, which the payload's `note` states; 400 `invalid ids` |
| `/api/functions/matches` | POST | the same read with the ids in the body; `{"function_ids": [...]}`; 400 `invalid ids` |
| `/api/functions/similar` | POST | rank the stored functions whose cached listing is most similar to a pasted query; body `{"listing": "..."}` or `{"bytes": "<hex>", "arch": "x86_32", "va": 0}` (bytes disassembled in process, never executed; `arch` one of `matching.QUERY_ARCHES`; at most 64 KiB of code or 1 MiB of listing) plus optional `min_similarity` (0-100, default 85) and `limit` (1-100, default 10); returns `{"function_id": null, "min_similarity", "limit", "candidate_source", "compared", "hits"}`, each hit `{"function_id", "name", "va", "binary_id", "binary_name", "similarity"}`, best first. Above 80 the candidates come from the LSH index (`candidate_source: "index"`), else every cached listing (`"pairwise"`); the hits are the same either way. Records nothing; 400 `invalid-similar-query` for a malformed or empty query, 503 `similarity-unavailable` without the extra |
...
| `/api/functions/<id>/explain/<domain>` | GET | match one explain domain (`crypto`, `execution`, `filesystem`, `networking`) against the same imports and literals, with the behavior rule table behind the binary-level scan; a deterministic text match, not the hosted portal's model narrative; 404 `domain not found` for an unknown domain, 404 `function not found` |
| `/api/functions/<id>/strings` | GET | the function's analyst strings, the quoted literals its stored decompilation carries, and stack-built or single-byte-XOR strings recovered from its stored NASM listing; the three halves are never merged and the payload's `note` says both scans are text, not engine output; 404 `function not found` |
...
| `/api/conversations/<id>/runs` | POST | run one agent turn: the model is offered every local MCP tool and may call them until it answers in text; body `{"content"}`; a read-only tool runs at once and a destructive one pauses the run; a second POST while a run is still live reuses that run without charging again; journaled (the run row and the messages it wrote), 503 `llm-unavailable`, 502 `llm-error`, 404 `conversation not found` |
...
| `/api/analyses/<id>/external/<source>` | POST | run one source for the analysis and store its answer as the `external:<source>` scan; 403 `external-disabled`, 503 `external-unavailable`, 400 `no-content-hash`, 502 `external-fetch-failed`, 404 `unknown source`, 429 `rate-limited` for a remote re-pull inside the hour; journaled |
...
| `/api/symbols/library` | GET | every symbol file in the workspace library, newest first, each with its `identity` (`pe:<guid>-<age>` or `elf:<build-id>`), kind, counts and origin; the stored parse stays off the list; stored-only |
| `/api/symbols/library` | POST | add symbol files to the workspace library: a multipart upload with one or more `file` parts, each parsed and identified before the first write, stored content-addressed under `<workspace>/symbols/<sha256>` and keyed by identity, all as one journaled action with already stored bytes reported `duplicate: true`; 400 `no-file`/`too-many-files`/`empty-file`/`symbols-unreadable`/`no-identity` |
| `/api/binaries/<id>/symbols/resolve` | POST | match the binary against the library by the identity its own bytes carry (the PE debug directory's CodeView GUID+age or the ELF GNU build id) and apply the match through the same import as one journaled action, the ingest under `ingest`; `?fetch=` absent asks the public symbol server for a missing PDB only while remote sources are enabled, `true` forces it (403 `external-disabled` when they are off), `false` stays local, any other value 400 `fetch must be a boolean`, and a failed fetch answers the external codes (`external-fetch-failed`/`external-bad-response`/`external-too-large`); no match answers 200 `matched: false` with the reason; 404 `binary not found` |
| `/api/binaries/<id>/decompiler-script` | GET | render the stored analysis as a runnable decompiler script: a Ghidra Python script (`?format=ghidra`, the default), an IDAPython script (`?format=ida`) or a Binary Ninja JSON document (`?format=binja`); `?include=` is a comma list of `renames` (the default), `comments`, `signatures` (complete stored prototypes only) and `summaries` (the stored AI summary, as part of the function comment); 400 `invalid format` or `invalid include`, 404 unknown binary; stored-only, placeholders and declaration words are never carried as names; answered as an attachment named `<binary stem>_renames_<tool>.py` (`.json` for Binja) |
...
| `/api/docs` | GET | every shipped documentation page in reading order, each with its `slug` and title; the pages are the markdown under the workspace's `docs/` (top level and one subdirectory level, so `docs/subsystems/store.md` has the slug `subsystems/store`) plus `CHANGELOG.md`; 404 `no-docs` when no documentation directory resolves (`REPORTAL_DOCS`, then the workspace, then the checkout) |
| `/api/docs/<slug>` | GET | one page parsed into its title, its `headings` (the on-this-page anchors), its `blocks` (headings, paragraphs, lists with each item's `depth`, fenced `code`, `quote`s and `table`s) and its `previous`/`next` neighbours in reading order (each a `slug` and title, null at either end), so the SPA renders it without any markup injection and without re-deriving the order; the slug is a path, so a subdirectory page reads `/api/docs/subsystems/store`; 404 `no-doc` for an unknown, malformed or escaping slug, `no-docs` when there is no directory |
...
| `/api/pipeline/runs/<id>/revert` | POST | replay the run's undo plan newest-first and return what was undone; 404 `run not found`, 409 `corrupt-undo-plan` when the stored plan is not a JSON array of objects |
| `/api/components` | GET | the component registry: each entry's `name`, `requires`, `provides`, `origin` and `reloadable` |
| `/api/components/reload` | POST | re-read one component's declaring module and swap the live registry entry; body `{"name": "..."}` or `{"all": true}`; tenant admin only (auth off is the local operator); 403 `forbidden` for a non-admin, 404 unknown name, 400 neither/both/non-boolean `all`, 409 `not-reloadable`, 500 `component-missing` |
| `/api/components/<name>/deactivate` | POST | withdraw one component from the live composition: run its `revert` where declared and revoke the names it provided; returns the `deactivated` entries and a `journal_action` when the withdrawal recorded a durable write; tenant admin only; 403 `forbidden` for a non-admin, 404 unknown name, 409 `not-withdrawable` (nothing to withdraw, or already withdrawn) |
...
| `/api/journal` | GET | recorded action-journal entries, newest first, without their descriptor payload; `?limit=` (default `journal.DEFAULT_LIST_LIMIT`, hard cap `journal.MAX_LIST_LIMIT`), optional `?action=` and optional `?actor=` (the identity the server set around the write, `local` while auth is off and empty for a CLI or MCP one); the body echoes what it applied and names the `actors` the journal holds; 400 `invalid limit` outside 1..`MAX_LIST_LIMIT` (a non-integer is `limit must be an integer`); read-only |
| `/api/jobs` | GET | queued and finished operations, newest first: `?status=` (one of `jobs.STATUSES`), `?kind=` (a registered job kind), `?binary_id=` and `?limit=` (bounded by `jobs.MAX_JOB_LIMIT`) are each validated with a 400; the payload carries the `jobs`, the matching `total`, the `queued` count and both closed vocabularies the filters accept, the `statuses` and the `kinds` that may be queued; a running `match` job reports its `progress` and `steps_total` in source functions and a running `ai-enrich` job in functions of the batch, every other kind is one step |
| `/api/jobs` | POST | queue one operation and answer `202` with the job; the registered kinds are `analyse` (generate the binary's rebrew project, import its functions and queue `jobs.analysis_follow_ups` for the target, answering their ids as `follow_up_jobs`; what an upload queues), the queued scans, `report` (the engine report over the reversed sources), `report-pdf` (the PDF over the stored scans), `match` (function matching against the corpus, which takes the match settings as its `params` and journals the rows it replaces) and `ai-enrich` (the AI enrichment chain over a binary's functions, which takes `{"limit"}` or `{"function_ids": [...]}` and stores one pipeline run per function, reporting the function count as its progress); body `{"kind", "binary_id", "params"?}` (an unknown kind, an unexpected parameter, a refused domain value, an out-of-range `limit` or a full queue is 400 `invalid job`, an unknown binary 404 `binary not found`); the server's bounded pool runs it, and when no pool can start the job runs inline |
...
| `/api/binaries/<id>/auto` | POST | plan an auto run and execute it in a background thread; body `{"worker", "execute", "concurrency", "functions_per_task", "max_attempts", "max_tasks", "goal", "max_tokens", "max_usd", "usd_per_mtok"}`, all optional and every bound validated (400 `invalid params`); a non-empty `goal` is the objective a goal-directed worker works toward and plans every function of the binary, matched ones included; answers 202 `{"run_id", "binary_id", "status": "running"}`; `execute` defaults to false; SaaS tenants past quota get 402 `quota-exceeded` |
...
| `/api/auto/runs/<id>/revert` | POST | remove the files the run wrote and whose bytes it still wrote (a path another writer replaced is reported `diverged` and kept), restore the statuses it changed and delete its rows; 404 `run not found`, 409 `corrupt-undo-plan` when the stored plan is not a JSON array of objects |
| `/api/auto/runs/<id>/recover` | POST | close a run a dead process left `running`: merge its unfinished tasks' recorded writes, mark them `interrupted`, and return the run id with the recovered and added-descriptor counts; a run already closed is returned unchanged with zeroes; 404 `run not found` |
...
| `/api/binaries/<id>/dynamic-execution/status` | GET | the analysis status read through the binary, for the binary detail's Sandbox panel: whether this install can detonate, the runner in use and the binary's last run; a binary with no analysis yet reports the opt-in state and no run; 404 `binary not found`; read-only |
...
| `/api/binaries/<id>/debug-session` | POST | run a read-only debug probe over a stored binary and store the transcript; body `{"timeout": seconds, "breakpoints": [va, ...], "qemu_arch": "x86_64"}` narrows the caps (`qemu_arch` runs the gdb probe under `qemu-<arch>` through `target remote`; 400 `invalid-debug` outside the caps, for a non-integer, or for a non-gdb backend); 403 `debug-disabled` unless the workspace opted in, 503 `debug-unavailable` without an installed backend, 404 `binary not found`, 400 `binary not on disk`; 201 with the finished session (the backend, the caps, the transcript), one journaled row; a second POST while a session is still `running` returns that live row as 202 without starting another probe |
| `/api/binaries/<id>/debug-session` | GET | the binary's newest debug session plus the `debug` status block; 404 `no-debug-session` before the first session, 404 `binary not found` for an unknown id |
| `/api/binaries/<id>/debug-session/status` | GET | whether this install can debug, the backend in use and the binary's last session; a binary with no analysis yet reports the opt-in state and no session; 404 `binary not found`; read-only |
| `/api/binaries/<id>/debug-coverage` | GET | which stored functions the newest debug session observed, joined by address (`observed` of `total`, per-function hits); stored-only, needs no backend and starts no probe; 404 `no-debug-session` before the first session, 404 `binary not found`; read-only |
| `/api/binaries/<id>/debug-proposals` | GET | rename proposals from the newest session's frame names, joined by address; a person-authored name is never proposed and nothing is renamed; 404 `no-debug-session` before the first session, 404 `binary not found`; read-only |
| `/api/functions/<id>/debug-apply` | POST | rename one function to its session proposal (body optional `{"name"}` override), recorded with source `debug` and journaled; 404 `function not found`, 400 `no-session-proposal` without a proposal, 400 `invalid name` for a non-string; 201 |
...
| `/api/analyses/<id>/requeue` | POST | put the analysis back to `pending`, clear its finish time, log the transition and queue a job for each stored scan that has a job kind (`jobs` on the body); a scan with no job kind stays a per-scan POST; journaled (a revert restores the status, the time, the log and the jobs); 404 `analysis not found` |
...
| `/api/analyses` | GET | analyses with the binary name, size, format, arch, its tags and its scope; `?status=` (any of `store.ANALYSIS_STATUSES`, repeatable and combined as any-of), `?search=` (the binary's name, its SHA-256 or a prefix of it, or the engine label), `?platform=`/`?arch=` (the stored binary's own `format`/`arch`), `?order=` (one of `store.ANALYSIS_ORDERS`: `newest`, `oldest`, `name`, `name-desc`, `size`, `size-desc`), `?limit=` (default and cap `store.MAX_ANALYSIS_LIMIT`), `?binary_id=` and `?workspace=personal\|team\|public` (one of `store.WORKSPACE_FILTERS`, reading the owning binary's scope: no owning team, an owning team, or one the whole workspace may see); an unknown value is 400; each row carries `visibility`, `owner_team_id` and `owner_team_name`, and the body carries `count` (returned), `total` (unfiltered, so a filter that matched nothing is distinguishable from an empty project), `statuses`, and the `platforms`/`architectures` the register actually holds, which is what the SPA's filter controls are built from |
...
| `/api/binaries/<id>/collections` | GET | the collections one binary is a member of, by name, each row carrying the collection's fields, its member count and its owning team's name; a collection the caller may not see is left out; 404 `binary not found` |
...
*i*'s `name`, `tags`, `collection_ids` and explicit `format`/`arch`/`compiler` (validated
against `UPLOAD_FORMATS`/`UPLOAD_ARCHITECTURES`/`UPLOAD_COMPILERS`, `auto` being absent), an entry
...
created.  A compiler *hint* on upload is not stored as the caller named it:
the recovered toolchain is stamped later from the filetype scan onto
`binaries.compiler`.  The hosted portal's Platform picker has no local column.
...
in text or the run reaches `agent.MAX_TOOL_CALLS`.  A second POST while a run is
still live returns that run without charging again or appending another user
message.  A tool that changes the
...
disagree.  The same names can come from the workspace library instead of one
upload: `GET`/`POST /api/symbols/library` stores vendor and system symbol files
keyed by identity, and `POST /api/binaries/<id>/symbols/resolve` applies the
entry whose identity the binary carries, asking the public symbol server for a
missing PDB only while remote sources are enabled.
...
`binary not found` for an unknown id, then computes and stores the ranking.
Without an engine, or for a binary the engine refuses to read, the scan
degrades to the stored fingerprints and `capabilities` scans and records a
note naming it, so this route is 200 either way.  `GET` on the same path is stored-only
...
already taken case-insensitively is 409 `duplicate family`, an unknown
...
The whole-binary form is not a route of its own: `pipeline.run_pipeline_batch`
loops the per-function composition over a binary's functions and a caller queues
it as the `ai-enrich` job kind, so `POST /api/jobs` with
`{"kind": "ai-enrich", "binary_id": N, "params": {"limit": 25}}` (or
`{"function_ids": [1, 2]}`) is the batch, and its result carries one run id per
function.
...
component (between `resolve-names` and `rewrite`, requiring `function` and
`conn`) provides `knowledge`: the binary's documents ranked against the
function's name, VA and stored summary.  It journals nothing, the provided
list is empty when the binary has no documents, and the enrich chain's stages
(`rewrite`, `rename-variables`, `name-variables` and `summarize`) add the
rendered block, plus the workspace's own known names (the predicted name, the
names an ingested debug symbol file carries for the function's VA and the
addresses its code references, and the function's match candidates), to their
prompts.
Retrieved text is
