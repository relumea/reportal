...
| `/api/organisations/<id>/billing/sync` | POST | re-read the org's subscription from the provider and mirror it; webhooks are the primary path and this only mirrors, so it cannot grant entitlement on its own; rate limited per organisation (429 past the cap) |
