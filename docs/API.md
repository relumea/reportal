...
| `/api/flirt/sigsets/refresh` | POST | index every `.sig` under the directory `REPORTAL_FLIRT_SIGS_DIR` names and nothing else: one row per file, an unchanged hash left alone, a changed one updated in place keeping its `enabled` flag; answers `added`, `updated`, `unchanged` and `pruned`; 400 `no signature directory` when the install configured none |
| `/api/binaries/<id>/flirt` | POST | match one binary against the enabled signatures for its architecture (body `{"arch"?}` overrides the stored fingerprint's) and cache the result under the signature library's key; 404 unknown binary, 400 `no signature directory` or `invalid arch` |
| `/api/binaries/<id>/flirt` | GET | the stored signature matches of one binary, without re-matching it: the matched names with their library, the `sigset_key` they were found under and `cached: true`; 404 `no flirt scan` before the first POST, 404 unknown binary |
