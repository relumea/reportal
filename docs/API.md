...
| `/api/users/<id>/oauth` | GET | every provider link the user holds; 404 `user-not-found`; read-only |
| `/api/users/<id>/oauth/<provider>` | DELETE | remove the user's link to one provider; 404 `user-not-found`, 409 `last-sign-in` when it is the last way in; journaled |
| `/api/oauth/providers` | GET | the configured sign-in providers (ids and labels, never secrets); an empty list means sign-in is off; read-only |
...
| `/api/health` | GET | status, version, database path, row counts, process-local `http` request counters (requests, 4xx/5xx, latency sum and max since start); every `/api` response echoes `X-Request-Id` (accepted when well-formed, otherwise minted); this is the running server's read, and `reportal doctor` is the pre-flight half (the SPA build, a free port, the schema and the auth posture before anything starts) |
...
| `/api/binary-options` | GET | every visible binary's id and name, for the views' binary pickers; stored-only and one statement, which is the point: a picker needs a label per binary, not the register's per-row SHA-256, size, scope and counts, and the caller's team scope applies exactly as it does on the register |
| `/api/binary-summaries` | GET | the dashboard's per-binary cards in one stored-only read: `?ids=1,2,3` names the binaries (at most `store.MAX_SUMMARY_BINARIES`), and each row carries the function total, the matched total, the status breakdown, the matched bytes inside each stored `pe-info` section, the stored report's byte coverage and the binary's latest auto run; ids that matched no binary are named in `missing`, duplicate ids answer once, no engine runs; 400 `invalid ids` outside the bounds |
