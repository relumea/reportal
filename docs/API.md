...
| `/api/jobs` | GET | queued and finished operations, newest first: `?status=` (one of `jobs.STATUSES`), `?kind=` (a registered job kind), `?binary_id=` and `?limit=` (bounded by `jobs.MAX_JOB_LIMIT`) are each validated with a 400; the payload carries the `jobs`, the matching `total`, the `queued` count and both closed vocabularies the filters accept, the `statuses` and the `kinds` that may be queued |
