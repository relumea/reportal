...
| Hot module replacement | `components.reload_component`, `components.reload_all`, `components.registrations`, `pipeline.ComponentHost` | Every registration records the module that declares it and whether it can be reloaded.  `reload_component` re-imports that module through `importlib.reload` (or re-scans the entry-point group) and swaps the registry entry in place, keeping declaration order; `reload_all` reports one result per component and skips the non-reloadable ones with a reason.  `ComponentHost` holds a live `Context` and drives deactivate → reload → activate for a component and its dependents, and `ComponentHost.sync` retires a fiber whose declaration has left the registry, since no later call can name it.  See the boundary below. |
| One effect mechanism (pipeline, auto mode and the action journal) | `effects.apply_undo_plan`; `effects.register_effect_handler`, `effects.effect_handlers`, `effects.unregister_effect_handler`, `effects.refresh_effect_handlers`, `effects.builtin_effect_handlers`; `pipeline._stored_plan`; `effects.EFFECT_CONTEXT_CHANGE`; `auto_mode.persist_undo_plan`, `auto_mode.recover_auto_run`, `auto_mode.revert_auto_run`; `auto_store.set_auto_run_effects`, `auto_store.record_auto_task_outcome`; `journal.Journal`, `journal.journaled`, `journal.revert_action`, `journal.revert_entry` | The dispatcher is the single place a descriptor kind maps to its inverse, and the kind-to-handler registry is what a third party extends without editing it.  Auto mode folds each task's descriptors into `auto_runs.effects_json` in the commit that records the task's result and reverts through `apply_undo_plan`; the pipeline journals during the run and reverts through `plan_context(...).revert()`; the action journal persists one request's descriptors in `journal_entries` and replays them through the same `apply_undo_plan`.  A pipeline run also stores a `context-change` descriptor per binding it made (`pipeline._stored_plan`): `revert_run` applies it against the run's live context where this process still holds it and reports it under `context_changes` otherwise. |
| Effect functions return their own inverse, per state (Section 3.1.2) | `components.Context.provide`, `Context.revoke`, `Context.record` | Each operation returns (here: journals) the inverse for the state it is applied at rather than a uniform `g` fixed in advance: `provide` captures the binding it replaced, `revoke` the binding it withdrew, `record` the descriptor a later process replays.  `Context.revert` is the accumulator, applied newest-first.  The paper's two motivations for the per-state inverse both hold: an inverse meets exactly the state its own application produced (Theorem 16), and one effect can be undone while the others are retained, which is what per-entry reverts (`journal.revert_entry`, `journal.revert_action`) and a component's own `Component.revert` do. |
| One writer per context name (Definitions 19 and 20; independence, Section 3.4.1) | `components.assert_unique_providers`; `pipeline.run_pipeline`, `pipeline.ComponentHost.__init__`, `ComponentHost.sync` | The paper's coeffect context makes a second provision of a bound key an error that produces no transition, and its composition guarantee needs exactly that: two writers of one key are not independent, and the second write is not the inverse of anything the first can undo.  The composition is checked before any effect runs, over the components the configuration leaves enabled (a disabled component is not a writer), and a reload is checked the same way before it is adopted. |
| Revert of a revert (Section 3.1.2, `effect_Γ`) | `journal.revert_action`, `journal.revert_entry`, `api.revert_data_type_history`, `api.revert_function_signature`; `journal.STATUS_ACTIVE`, `STATUS_REVERTED`, `STATUS_PARTIAL` | Undoing an effect is an effect in its own right.  Where a revert *writes rows*, that write is journaled in turn, so its own `journal_action` reverts the revert: the data-type and signature history reverts do exactly this.  Elsewhere the mechanism is the entry status: an entry moves `active` to `reverted` or `partial` and a repeat is `not-active`, which makes a revert idempotent rather than invertible.  A run-level revert (`pipeline.revert_run`, `auto_mode.revert_auto_run`) closes the run's rows instead of journaling its own inverse.  The gap is stated below. |
| Effect iterators (Section 3.1.3, Theorem 16, Definition 18) | `components.Context.provide`, `Context.revoke`, `Context.record`, `Context.revert`; `components.Context._effects`, `components.Context.load`, `components.EffectStep` | What a component loads by is a sequence of effects, and reverting it is the reverse-order pass Theorem 16 asks for: each operation appends its own inverse for the state it was applied at, and `Context.revert` walks that list newest-first, so every inverse meets exactly the state its own application produced and each intermediate state satisfies the invariant.  The sequence is the journal, one `Effect` per iteration, and `Context.load` hands it back as a chain of `EffectStep` values whose `rest` is the continuation and whose end is None. |
...
| Derived realization and realms (Section 3.2.3, Definitions 23 and 24) | `components.Context.derive`, `Context.drop`, `Context.parent`, `Context.spawn` | A context derives a child that reads the parent's bindings, writes into a table of its own and journals only its own effects, so one name resolves to different values in two contexts.  Deriving installs nothing on the parent, because its inverse is the identity; `drop` reverts the child's own journal and detaches it, which is how a recovery discards a realm. |
| Dependency interception (Section 3.2.3, Definition 28) | `components.Context.intercept`, `Context._resolve` | A hook installed for one name wraps every read of it through that context, after the parent's own hooks for a name the parent binds.  Installing an interception is a journaled effect of kind `intercept`, so `Context.revert` uninstalls it and the returned inverse removes it early. |
| Fibers (Section 4.1, Definition 49) | `components.Fiber`, `components.Context.spawn`, `pipeline.ComponentHost.fibers` | One instantiation carries its parent fiber, its own coeffect table (a context derived by default), a retirement flag and the four states `FIBER_PENDING`, `FIBER_ACTIVE`, `FIBER_RETIRED` and `FIBER_DISPOSED`.  `provides` commits each declared name to the fiber's committed view (its omega), `owner` resolves a name to the fiber that committed it, and the live composition hangs one fiber per activation off its root fiber.  A fiber created over a shared context (the host's) leaves the bindings to its owner; one that derived its own realm drops it on retirement. |
| Inertia and the retirement flag (Section 4.4, Table 2) | `components.Inertia`, `components.Fiber.retire`, `pipeline.withdraw_component` | `Fiber.retire` sets the retirement flag before it deactivates anything and returns an `Inertia` handle carrying the outcome, so a caller waits on the deactivations the withdrawal causes; a second call finds the transition in flight and returns the same handle.  The transition runs in the process that asked for it, so the handle is settled when `retire` returns and `wait` reports that instead of inventing a gap. |
| The effect iterator as a reified value (Section 3.1.3, Definitions 17 and 18) | `components.Context.load`, `components.EffectStep` | The journal as a value: each step carries the context it produced, the effect, that effect's inverse and the rest of the load, whose end is `None` (the paper's `Nothing`).  Walking the chain and applying each inverse in reverse order is what `Context.revert` does.  The witnessed variant is not implemented, because the witness is an author obligation (Section 5.1.1). |

### Paper mechanisms reportal does not implement

The map above is what reportal has; these are the paper's mechanisms it has no
counterpart for, each with what stands in their place.  The paper's own
implementation table (its Table 2) is the fuller cross-check, including the
names Cordis uses.

| Paper mechanism | What reportal has instead |
|-----------------|---------------------------|
| The witness on an effect function (Section 3.1.2) as a checked property | Unchecked, as it is in the paper's own implementation: an inverse is an obligation on the component author ("that the inverse reverts the effect it accompanies is an obligation on the component author rather than a property the runtime verifies", Section 5.1.1, delimited in Section 6.1).  reportal has no more than Cordis here, and the observational-equivalence gap below says so. |
...
and its `kind` must have a handler in the effects registry
(`reportal.effect_handlers`).
...
persistent write meant a new branch in an in-tree dispatch table; the
...
- **Hot module replacement replaces the registry entry, not a run in flight.**
  ...
  old bytecode.
- **A context name may be overwritten, where the paper's precondition refuses
  it.**  The paper's coeffect context requires a key not already bound for a provision
  and a key already bound for a restriction, and a violated precondition is an
  error that produces no transition (Definitions 19 and 20).  `Context.provide`
  instead accepts a second write to a bound name and journals the *restore* of
  the value it replaced, so a revert of two writes to one name walks back to the
  first value, and `Context.revoke` of an unbound name is a no-op that still
  notifies.  The composition-level half of the rule *is* enforced
  (`components.assert_unique_providers` refuses two enabled components
  providing one name, which is what the independence condition of Section 3.4.1
  needs); the operation-level half is deliberately looser, and its own tests
  pin that (`TestContext.test_provide_revert_restores_the_previous_binding`,
  `test_two_provides_of_one_name_revert_to_the_first_value`).  The consequence
  is stated rather than hidden: two writes to one name are a sequence whose
  revert is LIFO, not a pair of independent effects.
- **A revert is not always itself revertible.**  Where a revert writes rows it
  journals them, so the data-type and signature history reverts can be reverted
  in turn.  Elsewhere the entry status machine makes a revert idempotent (a
  second revert is `not-active`) rather than invertible, and a run-level revert
  closes the run's rows.  The paper's `effect_Γ` makes every revert an effect
  with its own inverse; reportal has that only on the paths whose revert writes
  something.
...
- **App-wide mediation is partial, and the wired set is explicit.**  The
  ...
  - **AI artifacts.**  Summary, inline comments and type suggestions, rename
    ...
    path), on the routes, CLI and MCP.  The enrich chain's order is a real edge
    in the graph rather than declaration order: `name-variables` and `summarize`
    require the `renamed_code` the `rename-variables` stage provides, so
    "summarize the renamed result" cannot be composed the other way round, and
    the `store` stage leaves a renamed decompilation alone instead of putting
    its own pre-rename context copy back.
  ...
  - **Run-scoped plans.**  The pipeline and auto mode keep their own plans by
    ...
    The whole-binary batch (`pipeline.run_pipeline_batch`, the `ai-enrich` job
    kind) is a loop over `run_pipeline` rather than one composition, so each
    function gets its own run-scoped plan and one function's artifacts are
    revertible without touching the others.
