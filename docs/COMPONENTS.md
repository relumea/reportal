...
| Effect iterators (Section 3.1.3, Theorem 16, Definition 18) | `components.Context.provide`, `Context.revoke`, `Context.record`, `Context.revert`; `components.Context._effects` | What a component loads by is a sequence of effects, and reverting it is the reverse-order pass Theorem 16 asks for: each operation appends its own inverse for the state it was applied at, and `Context.revert` walks that list newest-first, so every inverse meets exactly the state its own application produced and each intermediate state satisfies the invariant.  The sequence is the journal, one `Effect` per iteration, and its end is the list running out.  The iterator as a reified value (Definition 17's triple with a continuation) is not implemented; the table below says what stands in its place. |
...
|-----------------|---------------------------|
| Fibers (Section 4.1, Definition 49): a component instantiated many times over, each instance carrying its parent, its own coeffect table, a retirement flag and the four lifecycle states; the registry is a tree of them, and a fiber's committed view (its omega) records which fiber provided each declared key | One instance per component *name*, in one `Context` (`pipeline.ComponentHost._registered`/`_active`).  There is no instantiation, no parent tree, no per-fiber table and no committed view: `Context` holds the bindings, `ComponentHost` holds the active names and the per-name decisions, and a duplicate component name is refused at registration.  reportal composes a fixed pipeline of built-in stages rather than a system in which one component is instantiated repeatedly, so the fiber layer has nothing to describe here. |
| Coeffect isolation and interception (Section 3.2.3, Definitions 24 and 28): the same key resolves to different values under different contexts (realms), and dependency access carries cross-cutting behavior | One flat table.  `Context` is the single namespace a run reads and writes, and a version of it derived from another exists only as `effects.plan_context`'s reconstruction from a stored undo plan (a replay, not a child scope). |
| Derived realization (Section 3.2.3, Definition 23): a fresh context deriving from the input, with the identity as its inverse, so recovery discards it | In-place realization only: every operation mutates the one `Context` and journals the inverse that undoes the mutation.  Nothing in reportal derives a child context it can simply drop. |
...
| Inertia and the retirement flag (Section 4.4, Table 2): a handle on the transition in flight, so a withdrawal waits for the deactivations it causes | A withdrawal runs its deactivation and its dependents' synchronously (`ComponentHost.deactivate`/`withdraw_component`) and a swap applies to the next composition.  There is no in-flight handle, no `Future`, and no retirement flag: the run in flight keeps the snapshot it started with. |
| The effect iterator as a reified value (Section 3.1.3, Definitions 17 and 18): a recursive structure whose every step yields the new context, that step's inverse and a continuation (`Nothing` ends it), with the witnessed variant carrying the step's proof | The sequence is data, not a value an effect returns: `Context._effects` holds one `Effect` per operation and `Context.revert` performs the reverse-order pass over the list, so the continuation is the next list element and termination is the list ending.  A component writes its whole load through `Context` calls and declares `Component.revert` where the generic pass is not enough; nothing hands an iterator, a step or a continuation back to a caller, and the journal is not exposed as a traversal API. |
