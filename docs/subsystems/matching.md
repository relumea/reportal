...
Sources: src/reportal/matching.py, src/reportal/similarity.py, src/reportal/benchmark.py, src/reportal/library.py, src/reportal/flirt_sigs.py, src/reportal/unstrip.py

This subsystem owns scoring one function against a corpus and everything built on that score: the
match run, the assembly similarity core, benchmark scoring, library identification, the signature
catalog, the bill of materials and un-stripping. `matching.match_binary` is the
only scorer, disassembly and the scorer are injected, and a proposal is never applied by the read
that builds it.
...
- Routes: `POST /api/binaries/<id>/match`, `GET .../matches`, `POST .../matches/transfer`,
  ...
  `POST .../flirt/apply`.
- CLI: `match`, `function-matches`, `apply-match`, `library`, `sbom`, `unstrip`, `unstrip-apply`,
  `benchmark`, `benchmark-info`, `rename-benchmark`, `flirt`, `flirt-refresh`, `flirt-apply`.
- MCP: `get_matches`, `run_match`, `apply_match`, `get_library`, `run_library`, `export_sbom`,
  `get_unstrip`, `run_unstrip`, `apply_unstrip`, `get_benchmark`, `run_benchmark`,
  `get_rename_benchmark`, `get_flirt_sigsets`, `refresh_flirt_sigsets`, `get_flirt`, `run_flirt`,
  `apply_flirt`.
- Tables and scans: `matches` and `disasm_cache`; stored scans `library`, `unstrip`, `benchmark`
  and `flirt`; the catalog `sigset` and its scan cache `flirt_scan`.
