...
SETTINGS: tuple[Setting, ...] = (
    ...
    Setting(
        name="client.server",
        describe="the portal base URL the customer CLI calls",
        kind=KIND_TEXT,
        read=lambda: os.environ.get(customer_cli.SERVER_ENV, "").strip(),
        env=customer_cli.SERVER_ENV,
        default="the --server value",
    ),
    Setting(
        name="client.token",
        describe="the API token the customer CLI sends as a bearer",
        kind=KIND_SECRET,
        read=lambda: os.environ.get(customer_cli.TOKEN_ENV, "").strip(),
        env=customer_cli.TOKEN_ENV,
        default=NOT_SET,
    ),
    Setting(
        name="llm.endpoint",
    ...
    Setting(
        name="debug.enabled",
        describe="allow read-only debug sessions under an installed backend",
        kind=KIND_FLAG,
        read=debug.enabled,
        env=debug.ENABLED_ENV,
        table=debug.CONFIG_TABLE,
        key=debug.CONFIG_ENABLED,
        default="off",
    ),
    Setting(
        name="debug.backend",
        describe="the debug backend a session uses",
        kind=KIND_TEXT,
        read=debug.configured_backend_name,
        env=debug.BACKEND_ENV,
        table=debug.CONFIG_TABLE,
        key=debug.CONFIG_BACKEND,
