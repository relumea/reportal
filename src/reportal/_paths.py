...
# Workspace subdirectory holding the rebrew projects reportal generates for
# uploaded binaries (``rebrew_import.analyse_binary``), one per content hash.
# Derived from the binary, so a lost one is regenerated by analysing again.
...
# Workspace subdirectory holding git checkouts the agent may read and edit
# (``repos.py``).  Each checkout is one directory below it.
REPOS_DIR = "repos"
...
# (``docs.documents_dir``).
WORKSPACE_DIRS = (BINARIES_DIR, REPORTS_DIR, SYMBOLS_DIR, REPOS_DIR)
...
def projects_dir() -> Path:
    """Return the workspace directory holding generated rebrew projects.

    Beside the database (:func:`workspace_root`), so a process pinned by
    ``REPORTAL_DB`` keeps its projects with that database.  Not created here;
    the analysis that writes the first project creates it.
    """
    return workspace_root() / PROJECTS_DIR
...
def repos_dir() -> Path:
    """The workspace directory holding the git checkouts the agent may touch.

    :func:`workspace_root` answers, so a process pinned by ``REPORTAL_DB`` keeps
    its checkouts beside that database: the same bound :func:`under_workspace`
    checks and the bound a checkout path must stay inside.  The directory is not
    created here; ``reportal init`` creates it and the first clone fills it.
    """
    return workspace_root() / REPOS_DIR
